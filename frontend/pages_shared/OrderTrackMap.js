...
export default function OrderTrackMap({ order, driver }) {
  ...
  const dpos = (driver?.lat != null && driver?.lng != null)
    ? [Number(driver.lat), Number(driver.lng)]
    : null;

  const center = dpos || dropoff || pickup || [-1.2654, 116.8312];

  const line = [];
  if (dpos) line.push(dpos);
  if (!dpos && pickup) line.push(pickup);
